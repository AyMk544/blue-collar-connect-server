...
export const updateUserProfileById = async (c: Context): Promise<Response> => {
  return new Promise<Response>(async (resolve) => {
    ...
    bb.on('file', (fieldname: string, file: any, filename: any, encoding: string, mimetype: string) => {
      const safeFilename: string = typeof filename?.filename === 'string'
        ? filename.filename
        : typeof filename === 'string'
        ? filename
        : 'unknown_file';
    ...
    bb.on('finish', async () => {
      ...
      try {
        ...
        fields.updatedAt = admin.firestore.FieldValue.serverTimestamp() as unknown as string;
        await docRef.update(fields);
        resolve(c.json({ message: 'Profile updated successfully', updatedFields: fields }));
      } catch (error: any) {
